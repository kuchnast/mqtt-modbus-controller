...
public:
  DeviceController(const std::vector<DigitalInput> &inputs,
                   const std::vector<Relay> &relays,
                   const PollingConfig &polling_config, IModbusManager &modbus,
                   IMqttManager &mqtt);

  void poll_inputs();
  void process_relay_commands();
  void handle_mqtt_command(const std::string &topic,
                           const std::string &payload);
  void print_statistics();

  void start_watchdog(std::atomic<bool> &running,
...
private:
  ...
  struct RelayState {
    ...
    RelayState(const Relay *rel) : relay(rel), current_state(false) {}
  };
  ...
  void publish_input_state(InputState &state, bool current_state,
                           bool force = false);
