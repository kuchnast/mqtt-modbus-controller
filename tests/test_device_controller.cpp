...
TEST_F(DeviceControllerTest, PollInputsSuccess) {
    ...
    EXPECT_CALL(*mock_modbus_, read_discrete_inputs(1, 0, _))
        .WillOnce(Invoke(
            [input_states](int /*slave_id*/, int /*start_addr*/, std::array<uint8_t, 8>& dest) {
                dest = input_states;
                return true;
            }
        ));
...
TEST_F(DeviceControllerTest, PollInputsModbusFailure) {
    ...
    EXPECT_CALL(*mock_mqtt_, publish(_, _, _))
        .Times(0);
...
TEST_F(DeviceControllerTest, HandleRelayCommandWithRetry) {
    ...
    EXPECT_CALL(*mock_mqtt_, publish("test/relay1/state", _, _))
        .Times(0);
...
TEST_F(DeviceControllerTest, UnknownRelayCommand) {
    ...
    EXPECT_CALL(*mock_modbus_, write_coil(_, _, _))
        .Times(0);
    EXPECT_CALL(*mock_mqtt_, publish(_, _, _))
        .Times(0);
