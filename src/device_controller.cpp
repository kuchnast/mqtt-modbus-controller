...
DeviceController::DeviceController(const std::vector<DigitalInput> &inputs,
                                   const std::vector<Relay> &relays,
                                   const PollingConfig &polling_config,
                                   IModbusManager &modbus, IMqttManager &mqtt)
...
void DeviceController::process_relay_commands() {
...

  {
    ...
    const std::size_t count =
        std::min(relay_command_queue_.size(),
                 static_cast<size_t>(polling_config_.max_commands_per_cycle));
    std::move(relay_command_queue_.begin(),
              relay_command_queue_.begin() + count,
              std::back_inserter(commands));
    relay_command_queue_.erase(relay_command_queue_.begin(),
                               relay_command_queue_.begin() + count);

    if (!relay_command_queue_.empty()) {
      logger_.warning() << commands.size() << " commands in queue, limiting to "
                        << polling_config_.max_commands_per_cycle << "/cycle";
    }
  }
  ...
  for (const auto &cmd : commands) {
    ...
    if (it != relay_states_.end()) {
      ...
      if (modbus_.write_coil(state.relay->slave_id, state.relay->address,
                             cmd.desired_state)) {

        state.current_state = cmd.desired_state;
        publish_relay_state(state);

        logger_.debug() << "RELAY: " << state.relay->name << " @ slave "
                        << state.relay->slave_id << " addr "
                        << state.relay->address << " = "
                        << (cmd.desired_state ? "ON" : "OFF");
      } else {
...
void DeviceController::handle_mqtt_command(const std::string &topic,
...
void DeviceController::print_statistics() {
  ...
  auto elapsed =
      std::chrono::duration_cast<std::chrono::seconds>(now - last_stats_time_)
          .count();
  ...
  logger_.debug() << "Modbus Reads: " << modbus_stats->read_success << "/"
                  << total_reads
                  << (total_reads > 0
                          ? " (" +
                                std::to_string(100.0 *
                                               modbus_stats->read_success /
                                               total_reads) +
                                "%)"
                          : "");
  logger_.debug() << "Modbus Writes: " << modbus_stats->write_success << "/"
                  << total_writes
                  << (total_writes > 0
                          ? " (" +
                                std::to_string(100.0 *
                                               modbus_stats->write_success /
                                               total_writes) +
                                "%)"
                          : "");
  logger_.debug() << "MQTT Publishes: " << mqtt_stats->publish_success << "/"
                  << total_mqtt
                  << (total_mqtt > 0
                          ? " (" +
                                std::to_string(100.0 *
                                               mqtt_stats->publish_success /
                                               total_mqtt) +
                                "%)"
                          : "");
  logger_.debug() << "MQTT Messages Received: "
...
void DeviceController::start_watchdog(std::atomic<bool> &running,
                                      std::atomic<bool> &force_exit) {
  std::thread watchdog([this, &running, &force_exit]() {
    logger_.info() << "Watchdog: started (alarm after "
                   << polling_config_.watchdog_timeout_sec << "s)";
    ...
    while (running && !force_exit) {
      ...
      if (elapsed > polling_config_.watchdog_timeout_sec && running) {
        ...
        logger_.critical() << "Main loop not responding for " << elapsed
                           << "s!";
...
void DeviceController::publish_input_state(InputState &state,
                                           bool current_state, bool force) {
  auto now = std::chrono::steady_clock::now();
  auto elapsed =
      std::chrono::duration_cast<std::chrono::seconds>(now - state.last_publish)
          .count();

  bool should_publish = force || (current_state != state.last_state) ||
...
void DeviceController::group_inputs_by_slave(
    std::map<int, std::vector<InputState *>> &groups) {
